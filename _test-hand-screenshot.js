...
(async () => {
  ...
  await page.goto('http://localhost:8765/arabic-hand-tattoo.html', { waitUntil: 'networkidle0' });
  await new Promise(r => setTimeout(r, 2000));
  for (const [id, tag] of [['placement','placement'],['finger','finger']]) {
    await page.evaluate(i => document.getElementById(i).scrollIntoView({ block: 'start' }), id);
    await new Promise(r => setTimeout(r, 500));
    await page.screenshot({ path: OUT + `hand-${tag}.png`, fullPage: false });
  }
