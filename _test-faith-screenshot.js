...
(async () => {
  ...
  await page.goto('http://localhost:8765/faith-arabic-tattoo.html', { waitUntil: 'networkidle0' });
  await new Promise(r => setTimeout(r, 2000));
  for (const [id, tag] of [['faith-hope-love','fhl'],['walk-by-faith','walk'],['christian-phrases','christian']]) {
    await page.evaluate(i => document.getElementById(i).scrollIntoView({ block: 'start' }), id);
    await new Promise(r => setTimeout(r, 500));
    await page.screenshot({ path: OUT + `faith-${tag}.png`, fullPage: false });
  }
