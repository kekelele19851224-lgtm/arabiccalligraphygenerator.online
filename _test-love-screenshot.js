...
(async () => {
  ...
  await page.goto('http://localhost:8765/love-arabic-tattoo.html', { waitUntil: 'networkidle0' });
  await new Promise(r => setTimeout(r, 2000));
  await page.screenshot({ path: OUT + 'love-01-top.png', fullPage: false });
  for (const [id, tag] of [['words','words'],['love-yourself-first','lyf'],['designs','designs'],['choose','choose']]) {
    await page.evaluate(i => document.getElementById(i).scrollIntoView({ block: 'start' }), id);
    await new Promise(r => setTimeout(r, 500));
    await page.screenshot({ path: OUT + `love-${tag}.png`, fullPage: false });
  }
