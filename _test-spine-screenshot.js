...
(async () => {
  ...
  await page.goto('http://localhost:8765/arabic-spine-tattoo.html', { waitUntil: 'networkidle0' });
  await new Promise(r => setTimeout(r, 2000));
  await page.screenshot({ path: OUT + 'spine-01-top.png', fullPage: false });
  for (const [id, tag] of [['words','words'],['quotes','quotes'],['writing','writing'],['meaning','meaning']]) {
    await page.evaluate(i => document.getElementById(i).scrollIntoView({ block: 'start' }), id);
    await new Promise(r => setTimeout(r, 500));
    await page.screenshot({ path: OUT + `spine-${tag}.png`, fullPage: false });
  }
