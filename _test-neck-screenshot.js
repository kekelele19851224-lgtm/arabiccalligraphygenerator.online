...
(async () => {
  ...
  await page.goto('http://localhost:8765/arabic-neck-tattoo.html', { waitUntil: 'networkidle0' });
  await new Promise(r => setTimeout(r, 2000));
  await page.screenshot({ path: OUT + 'neck-01-top.png', fullPage: false });
  for (const [id, tag] of [['words','words'],['designs','designs'],['side','side'],['men','men']]) {
    await page.evaluate(i => document.getElementById(i).scrollIntoView({ block: 'start' }), id);
    await new Promise(r => setTimeout(r, 500));
    await page.screenshot({ path: OUT + `neck-${tag}.png`, fullPage: false });
  }
