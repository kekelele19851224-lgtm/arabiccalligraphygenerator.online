...
const phrasesLinksHtml = data.phrases
  .map(p => `        <a href="${p.url}" role="menuitem">${p.displayName}</a>`)
  .join('\n');

// 用于 footer 列表（含缩进 10 空格）
const phrasesListItemsHtml = data.phrases
  .map(p => `          <li><a href="${p.url}">${p.displayName}</a></li>`)
  .join('\n');

// ========== 替换模板占位符 ==========
const navRendered = navTemplate.replace('{{PHRASES_LINKS}}', phrasesLinksHtml);
const footerRendered = footerTemplate.replace('{{PHRASES_LIST_ITEMS}}', phrasesListItemsHtml);
